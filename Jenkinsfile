pipeline {
  ...
  stages {
    ...
    stage('Docker deploy') {
      ...
      steps {
        sh 'ansible all --become-method sudo -b -i "$LB_IP:50001, $LB_IP:50002, $LB_IP:50003",\
        ...
        // sh 'ansible all --become-method sudo -b -i "host1 ansible_host=$LB_IP ansible_port=50001, host2 ansible_host=$LB_IP ansible_port=50002, host3 ansible_host=$LB_IP ansible_port=50003",\
        //  -u $deployment_group_cred_USR -e "ansible_password=$deployment_group_cred_PSW" \
        //  -m shell -a "docker rm -f petclinic || true"'
        // sh 'ansible all --become-method sudo -b -i "host1 ansible_host=$LB_IP ansible_port=50001, host2 ansible_host=$LB_IP ansible_port=50002, host3 ansible_host=$LB_IP ansible_port=50003",\
        //  -u $deployment_group_cred_USR -e "ansible_password=$deployment_group_cred_PSW" \
